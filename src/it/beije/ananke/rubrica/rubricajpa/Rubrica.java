...
import it.beije.ananke.rubrica.rubricahbm.csvmanager.CSVManager;
import it.beije.ananke.rubrica.rubricahbm.hbmanager.HBManager;
import it.beije.ananke.rubrica.rubricahbm.xmlmanager.XMLManager;
...
public class Rubrica {
	...
	public List<Contact> importFromDB() {
		this.contacts = JPAManager.selectAll();
		return contacts;
	}
	...
	public void exportAsXML(String root, List<Contact> contacts)
	...
	public void orderByName() throws IOException, TransformerException, ParserConfigurationException {
		Collections.sort(contacts, Comparator.comparing(Contact::getFirstName)); // (c1,c2)->
																					// c1.getFirstName().compare(c2.getFirstName());
	...
	public void orderByLastName() throws IOException, TransformerException, ParserConfigurationException {
		Collections.sort(contacts, Comparator.comparing(Contact::getLastName)); // (c1,c2)->
																				// c1.getFirstName().compare(c2.getFirstName());
		printContacts(contacts);
		exportAsCSV(CSVroot, contacts);
		exportAsXML(XMLroot, contacts);
					...
					case 1:
						while (true) {
							try {
								...
								newFirstName = newFirstName.substring(0, 1).toUpperCase()
					...
					case 2:
						while (true) {
							try {
								...
								newLastName = newLastName.substring(0, 1).toUpperCase()
		...
		do {
			...
			case 1:
				...
				do {
					...
					contacts = HBManager.selectByField(field, firstName.toString());
			...
			case 2:
				...
				do {
					...
					contacts = HBManager.selectByField(field, lastName.toString());
					System.out.println("Found? [yes/no]");
					if (scanner.nextLine().equalsIgnoreCase("yes")) {
						found2 = true;
					} else {
			...
			case 3:
				...
				do {
					...
					contacts = HBManager.selectByField(field, phoneNumber.toString());
					System.out.println("Found? [yes/no]");
					if (scanner.nextLine().equalsIgnoreCase("yes")) {
						found3 = true;
					} else {
			...
			case 4:
				...
				do {
					...
					contacts = HBManager.selectByField(field, email.toString());
					System.out.println("Found? [yes/no]");
					if (scanner.nextLine().equalsIgnoreCase("yes")) {
						found4 = true;
					} else {
	...
	public Contact getContact(Scanner scanner) {
		...
		contactsFound = JPAManager.selectByField("id", id);
